---
name: release-br-openspec
description: >-
  Use this skill when releasing BR-OpenSpec: audit merged work and changeset
  coverage, decide whether a catch-up changeset PR is needed, prepare or resume
  the Changesets Version Packages PR, cut a stable release, verify publishing,
  and polish GitHub release notes. Also use when asked whether an open release
  PR is complete, what the next release step is, or to continue a release
  paused for human approval.
---

# Release BR-OpenSpec

Run the BR-OpenSpec release workflow as a resumable state machine. Inspect live GitHub state on every invocation and take only the next safe action. Do not assume an earlier invocation completed.

## Principles

- Treat `dynamicworks-com-br/BR-OpenSpec` and `origin/main` as the release source of truth.
...
1. Verify the repository and tools:
   ...
   - Stop before release mutations if the canonical repository is not `dynamicworks-com-br/BR-OpenSpec`.
...
3. Find the latest stable GitHub release. Exclude drafts and prereleases; do not use `git describe`, which may resolve to a tag that is not the stable release.

   ```bash
   gh release list --repo dynamicworks-com-br/BR-OpenSpec \
...
4. Find open release-related PRs:
   ...
   ```bash
   gh pr list --repo dynamicworks-com-br/BR-OpenSpec --state open \
...
5. Read the live release policy in `.changeset/README.md`, pending `.changeset/*.md` files on `origin/main`, and the Version Packages PR body/files when it exists. See `DEPLOY.md` for the fork's CI, secrets, and npm trusted-publishing setup.
...
5. Use the exact package name `"@dynamicworks/br-openspec"`, the highest required semantic bump, only relevant headings, and user-focused descriptions. Remember that changeset descriptions for this fork are written in Brazilian Portuguese.
...
1. Confirm it targets `main` from `changeset-release/main` and is generated by the `release-prepare.yml` Changesets automation (which authenticates with a GitHub App token so that CI runs on the version PR).
...
1. Find the `release-prepare.yml` workflow run for the merge commit and wait for completion. On merge, the Changesets action publishes via `pnpm run release:ci` (a pack-version check followed by `changeset publish`) with `NPM_TOKEN`/`NODE_AUTH_TOKEN` and `NPM_CONFIG_PROVENANCE=true` on Node 22, and creates the GitHub Release.
2. Verify all three artifacts independently:
   - `npm view @dynamicworks/br-openspec@<version> version`
