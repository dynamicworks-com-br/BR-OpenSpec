...
1. Bind the release values once and fetch the current release. Replace the example values, but keep every expansion quoted:
   ...
   tag="vX.Y.Z"
   previous_tag="vA.B.C"
   gh release view "$tag" --repo dynamicworks-com-br/BR-OpenSpec \
     --json body,name,isPrerelease,url
   ```

2. Find the preceding stable release by excluding drafts and prereleases.
3. Fetch GitHub-generated notes to recover first-time contributor attribution and the full changelog link:

   ```bash
   gh api repos/dynamicworks-com-br/BR-OpenSpec/releases/generate-notes \
...
- Write for developers using BR-OpenSpec with AI coding assistants.
...
- Exclude core maintainer `@fkmatsuda` from `New Contributors`. If no external first-time contributors remain, omit that section.
...
gh release edit "$tag" --repo dynamicworks-com-br/BR-OpenSpec \
