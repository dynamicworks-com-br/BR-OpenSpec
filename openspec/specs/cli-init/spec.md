...
- **AND** spell each command the way the configured tool registers it: `/opsx-<id>` for tools whose command files are named `opsx-<id>`, and the tool's skill invocation (`/skill:openspec-<skill>` for Kimi Code, `$openspec-<skill>` for Codex skills, `/openspec-<skill>` otherwise) for tools that receive no command files
