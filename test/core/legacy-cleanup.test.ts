...
describe('legacy-cleanup', () => {
  ...
  beforeEach(async () => {
    ...
    // A detecção/limpeza de prompts globais varre `<CODEX_HOME>/prompts`;
    // isolar dentro do testDir para nunca tocar o `~/.codex` real.
...
${OPENSPEC_MARKERS.end}`);
  ...
  describe('detectLegacyArtifacts', () => {
    ...
    it('should detect the fork-only opsx-code-review.md global prompt', async () => {
      // Divergência do fork: o adapter do BR-OpenSpec gerava opsx-code-review.md
      // (workflow `code-review`), que não existe na allowlist do upstream.
      const promptDir = getCodexPromptDir();
      await fs.mkdir(promptDir, { recursive: true });
      await fs.writeFile(path.join(promptDir, 'opsx-code-review.md'), 'legacy code-review prompt');

      const result = await detectLegacyArtifacts(testDir);

      expect(result.globalSlashCommandFiles).toContain(path.join(promptDir, 'opsx-code-review.md'));
    });
...
${OPENSPEC_MARKERS.end}`);
    ...
    it('should skip unmanaged global prompt paths in stale detection objects', async () => {
      ...
      expect(result.errors).toContain(LEGACY_CLEANUP_MESSAGES.skippedUnmanagedGlobalPrompt(unmanagedPrompt));
      expect(result.errors).toContain(LEGACY_CLEANUP_MESSAGES.skippedUnmanagedGlobalPrompt(outsidePrompt));
      await expect(fs.access(managedPrompt)).rejects.toThrow();
      await expect(fs.access(unmanagedPrompt)).resolves.not.toThrow();
  ...
  describe('formatCleanupSummary', () => {
    ...
    it('should format deleted directories', () => {
      ...
      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removido .claude/commands/openspec/ (substituído por skills e comandos do BR-OpenSpec)');
    });
    ...
    it('should format deferred global prompts cleanup separately from repo-local files', () => {
      ...
      const detection = {
        ...
        globalSlashCommandDetails: [{
          ...
          workflowIds: ['explore' as const],
          replacementLabel: LEGACY_CLEANUP_MESSAGES.codexSkillsReplacementLabel,
        }],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
      ...
      expect(summary).toContain(LEGACY_CLEANUP_MESSAGES.deferredGlobalPromptsHeader);
      expect(summary).toContain('só serão removidos depois que as skills substitutas');
  ...
  describe('LEGACY_SLASH_COMMAND_PATHS', () => {
    ...
    it('should only include legacy tool IDs with a command surface capability', () => {
      ...
      for (const tool of Object.keys(LEGACY_SLASH_COMMAND_PATHS)) {
        expect(registeredTools.has(tool) || resolveCommandSurfaceCapability(tool) === 'skills-invocable', tool).toBe(true);
      }
  ...
  describe('LEGACY_GLOBAL_SLASH_COMMAND_PATHS', () => {
    it('should define the allowlisted managed global Codex prompt names separately from project-local paths', () => {
      ...
      // Divergência do fork: 13 workflows, incluindo `code-review`.
      expect(codexPatterns.managedFileNames).toContain('opsx-code-review.md');
      expect(codexPatterns.workflowIdsByFileName?.['opsx-update.md']).toEqual(['update']);
      expect(codexPatterns.workflowIdsByFileName?.['opsx-code-review.md']).toEqual(['code-review']);
