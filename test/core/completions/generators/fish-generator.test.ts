...
import { CLI_DESCRIPTIONS } from '../../../../src/messages/index.js';
  ...
  describe('generate', () => {
    it('should generate valid fish completion script with header', () => {
      const commands: CommandDefinition[] = [
        {
          name: 'init',
          description: 'Initialize BR-OpenSpec',
          flags: [],
        },
      ];
      ...
      expect(script).toContain('# Fish completion script for BR-OpenSpec CLI');
    ...
    it('should generate helper functions for Fish', () => {
      ...
      expect(script).toContain(
        `complete -c openspec -l no-color -f -d '${CLI_DESCRIPTIONS.noColor}'`
      );
    ...
    it('should include all commands with descriptions', () => {
      ...
      expect(script).toContain("'Initialize BR-OpenSpec'");
    ...
    // upstream: 'should force path completion for every registry-backed path flag'
    // — varre o COMMAND_REGISTRY atrás de `-l path` / `-l code-workspace` /
    // `-l member`, que pertencem a `store`/`context`/`workset` (subsistema de
    // stores, adiado no fork). Sem essas entradas no registry o teste não teria
    // nenhuma linha para inspecionar. O caso equivalente com fixture inline
    // ('should force file completion for path flags…') fica logo acima.
    ...
    it('should handle indexed positional arguments for schema fork', () => {
      ...
      // O fork não tem `__complete schemas` nem o helper
      // `__fish_openspec_schemas`, então 'schema-name' cai no `default` do
      // gerador: sem sugestões dinâmicas, mas com -f (nada de arquivos).
      expect(sourceLine).toBeDefined();
      expect(sourceLine).toContain('-f');
      expect(sourceLine).not.toContain('__fish_openspec_schemas');
