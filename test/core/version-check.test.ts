...
describe('getAvailableCliUpdate', () => {
  ...
  it('asks the dist-tag endpoint, and never with an Accept type it answers 406 for', async () => {
    ...
    expect(requests[0].url).toBe('/@dynamicworks/br-openspec/latest');
  ...
  it('follows a redirect, as mirrors and corporate front-ends send', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/elsewhere/@dynamicworks/br-openspec/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
    ...
    await expect(getAvailableCliUpdate()).resolves.toBe(bumpMajor(OPENSPEC_VERSION));
    expect(requests[1].url).toBe('/elsewhere/@dynamicworks/br-openspec/latest');
  });
  ...
  it('tears down a redirected connection when the overall budget expires', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/mirror/@dynamicworks/br-openspec/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
  ...
  it('asks the registry npm exported, and only that', () => {
    ...
    try {
      ...
      expect(registryUrl()).toBe('https://registry.npmjs.org/@dynamicworks/br-openspec/latest');

      process.env.npm_config_registry = 'https://env.example.com';
      expect(registryUrl()).toBe('https://env.example.com/@dynamicworks/br-openspec/latest');
    } finally {
  ...
  it('falls back to the public registry when the override is not an http(s) URL', () => {
    ...
    expect(registryUrl()).toBe('https://npm.internal.example.com/@dynamicworks/br-openspec/latest');
  });
});
...
describe('offerCliUpgrade', () => {
  ...
  it('offers only for an npm-owned global install', () => {
    ...
    const npmGlobal = path.join(npmGlobalRoots()[0], '@dynamicworks', 'br-openspec');
    ...
    const notOurs = [
      ...
      path.join(PROJECT_ROOT, 'node_modules', '@dynamicworks', 'br-openspec'),
      null,
    ];
  ...
  it('recognizes an npm prefix that the node binary does not point at', () => {
    ...
    try {
      ...
      const installed = isWindows
        ? path.join(prefix, 'node_modules', '@dynamicworks', 'br-openspec')
        : path.join(prefix, 'lib', 'node_modules', '@dynamicworks', 'br-openspec');
      ...
      try {
        const copied = isWindows
          ? path.join(portable, 'node_modules', '@dynamicworks', 'br-openspec')
          : path.join(portable, 'lib', 'node_modules', '@dynamicworks', 'br-openspec');
  ...
  it('recognizes npm global roots without shelling out', () => {
    ...
    expect(isNpmGlobalInstall(path.join(roots[0], '@dynamicworks', 'br-openspec'), roots)).toBe(true);
...
describe('rerunUpdateWithUpgradedCli', () => {
  ...
  it('reports a failure when there is no upgraded CLI to hand off to', async () => {
    ...
    expect(lines.join('\n')).toContain('não foram regenerados');
  }, 30000);
});
...
describe('displayCliUpdateNote', () => {
  ...
  it('names the global install command and the copy that answered', () => {
    ...
    expect(output).toContain('npm install -g @dynamicworks/br-openspec@latest');
    expect(output).toContain('Depois execute "openspec update" novamente');
    expect(output).toContain(`Executando a partir de: ${getInstallDir()}`);
  });

  it('picks the upgrade command that matches how the CLI was installed', () => {
    const globalDir = path.join(GLOBAL_ROOT, 'lib', 'node_modules', '@dynamicworks', 'br-openspec');
    const globalLines = buildCliUpdateLines('9.9.9', globalDir, PROJECT_ROOT).join('\n');
    expect(globalLines).toContain('npm install -g @dynamicworks/br-openspec@latest');
    ...
    expect(local).toContain('Atualize a dependência @dynamicworks/br-openspec neste projeto.');
    expect(local).not.toContain('npm install');

    const npx = buildCliUpdateLines(
      '9.9.9',
      path.join(GLOBAL_ROOT, '.npm', '_npx', 'abc123', 'node_modules', '@dynamicworks', 'br-openspec'),
      PROJECT_ROOT
    ).join('\n');
    expect(npx).toContain('npx @dynamicworks/br-openspec@latest update');
  ...
  it('recognizes project-local installs from any directory under the project', () => {
    ...
    expect(
      isProjectLocalInstall(
        path.join(GLOBAL_ROOT, 'lib', 'node_modules', '@dynamicworks', 'br-openspec'),
        PROJECT_ROOT
      )
    ).toBe(false);
  ...
  it('does not tell npx users to run an update they were just handed', () => {
    ...
    expect(npx).toEqual(['  npx @dynamicworks/br-openspec@latest update']);
    ...
    expect(buildUpgradeCommandLines(path.join(GLOBAL_ROOT, 'lib', 'node_modules', 'pkg'), PROJECT_ROOT))
      .toContain('  Depois execute "openspec update" novamente para aplicar os novos fluxos de trabalho.');
  });
