...
import { SPECS_APPLY_MESSAGES } from '../../src/messages/index.js';
...
describe('findPurposePlaceholderIssue', () => {
  describe('the writer and the detector share one definition', () => {
    it('composes the archive placeholder from the same halves the detector reads', () => {
      // Guard against drift: the catalog is the single definition of the
      // placeholder text, and constants.ts must re-export exactly its halves.
      expect(SPECS_APPLY_MESSAGES.skeletonPurpose('x')).toBe(
        `${PURPOSE_PLACEHOLDER_PREFIX}x${PURPOSE_PLACEHOLDER_SUFFIX}`
      );
      expect(PURPOSE_PLACEHOLDER_PREFIX).toBe(SPECS_APPLY_MESSAGES.skeletonPurposePrefix);
      expect(PURPOSE_PLACEHOLDER_SUFFIX).toBe(SPECS_APPLY_MESSAGES.skeletonPurposeSuffix);
    });

    it('keeps the placeholder text the fork has always written', () => {
      // archive.test.ts asserts this literally too; pinned here so a reworded
      // placeholder is a deliberate change in both places.
      expect(SPECS_APPLY_MESSAGES.skeletonPurpose('add-retry-budget')).toBe(
        'A definir - criado ao arquivar alteração add-retry-budget. Atualize o Purpose após o arquivamento.'
      );
    });
  });
  ...
  describe('reports a placeholder', () => {
    ...
    it('reports a leading "A definir" marker, the fork\'s own wording', () => {
      // The fork's schema.yaml and docs tell agents to leave `A definir` as the
      // brief placeholder; a Purpose that is only that is as unwritten as `TBD`.
      expect(findPurposePlaceholderIssue('A definir', specWith('A definir'))).toEqual({ line: 4 });
    });

    it('reports an "A definir" opening a longer placeholder sentence', () => {
      const purpose = 'A definir: descrever esta capability.';
      expect(findPurposePlaceholderIssue(purpose, specWith(purpose))).toEqual({ line: 4 });
    });

    it('ignores case in a leading "a definir" too', () => {
      const purpose = 'a definir - escrever depois';
      expect(findPurposePlaceholderIssue(purpose, specWith(purpose))).not.toBeNull();
    });

    it('still reports the upstream English placeholder when it opens the Purpose', () => {
      // A project that started on upstream OpenSpec carries the English
      // sentence; it opens with `TBD`, so the marker rule covers it.
      const purpose = 'TBD - created by archiving change x. Update Purpose after archive.';
      expect(findPurposePlaceholderIssue(purpose, specWith(purpose))).toEqual({ line: 4 });
    });
  ...
  describe('leaves authored prose alone', () => {
    ...
    it('does not report a Portuguese word that merely starts with the marker', () => {
      // "A definição…" shares its first letters with "A definir" but is not it;
      // "A definirmos…" is the marker followed by a letter, which the lookahead
      // rejects. Neither is an unwritten Purpose.
      for (const purpose of [
        'A definição do orçamento de retry fica a cargo desta capability, que limita as tentativas.',
        'A definirmos o orçamento, esta capability limita quantas vezes uma entrega é reenviada.',
      ]) {
        expect(findPurposePlaceholderIssue(purpose, specWith(purpose))).toBeNull();
      }
    });

    it('does not report "a definir" raised mid-sentence', () => {
      const purpose =
        'Limita quantas vezes uma entrega falha é reenviada. O orçamento exato fica a definir após os testes de carga.';
      expect(findPurposePlaceholderIssue(purpose, specWith(purpose))).toBeNull();
    });
    ...
    it('still reads the punctuation a marker is written with', () => {
      ...
      for (const purpose of [
        'TODO(owner): describe this.',
        'TBD.',
        'TBD, pending a rewrite.',
        'A definir.',
        'A definir, ainda.',
      ]) {
        expect(findPurposePlaceholderIssue(purpose, specWith(purpose))).not.toBeNull();
  ...
  describe('the reported line follows the placeholder, wherever it sits', () => {
    const cases: Array<{ name: string; purpose: string; line: number }> = [
      ...
      {
        // Only passes because `A definir` is a leading marker: rewrapped, the
        // generated sentence no longer sits on one line, so the constants
        // cannot locate it and the marker rule has to.
        name: 'generated sentence rewrapped across two lines by a formatter',
        purpose: 'A definir - criado ao arquivar\nalteração c1. Atualize o Purpose após o arquivamento.',
        line: 4,
      },
    ];
    ...
    for (const { name, purpose, line } of cases) {
      it(name, () => {
        ...
        expect(/^(?:TBD|A definir)/.test(onThatLine)).toBe(true);
