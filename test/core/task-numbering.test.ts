...
import { TASK_NUMBERING_MESSAGES } from '../../src/messages/index.js';
...
describe('findTaskNumberingIssues', () => {
  it('matches duplicate ids at full depth', () => {
    ...
    expect(issues).toEqual([
      {
        line: 4,
        message: TASK_NUMBERING_MESSAGES.duplicateTaskId(
          '3.2.1',
          TASK_NUMBERING_MESSAGES.firstDeclaredOnLine(2)
        ),
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        message: TASK_NUMBERING_MESSAGES.duplicateTaskId(
          '2.1',
          TASK_NUMBERING_MESSAGES.firstDeclaredInFileOnLine('backend/tasks.md', 2)
        ),
      },
    ]);
  });
