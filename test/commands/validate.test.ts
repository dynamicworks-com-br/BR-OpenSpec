...
describe('top-level validate command', () => {
  ...
  it('errors on ambiguous item names and suggests type override', async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('é ambíguo');
  });
  ...
  it('resolves and validates a scaffolded change without proposal.md (#1182)', async () => {
    ...
    expect(result.stderr).not.toContain('Item desconhecido');
    expect(result.exitCode).toBe(0);
  });

  it('a resolved-but-invalid proposal-less change exits non-zero, not "Item desconhecido" (#1182)', async () => {
  ...
  it('includes a sole proposal-less change in --all (not "Nenhum item encontrado") (#1182)', async () => {
    ...
    try {
      ...
      expect(result.stdout + result.stderr).not.toContain('Nenhum item encontrado para validar.');
      expect(result.exitCode).toBe(0);
    } finally {
