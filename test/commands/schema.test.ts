        ...
        issues: [
          {
            ...
            message: expect.stringContaining('fora do diretório de templates do esquema'),
          },
        ],
      });
    });
...
`
      ...
      expect(JSON.parse(output as string).error).toContain(
        'Não é possível copiar o esquema com uma entrada vinculada'
      );
      expect(JSON.parse(output as string).error).toContain('fora do diretório permitido');
    });
...
`
  ...
  describe('schema init', () => {
    ...
    it('uses the configured default in the next new change command', async () => {
      ...
      // O fork ainda não tem --json em `new change` (upstream tem); a asserção
      // seguinte lê o .openspec.yaml, não o stdout.
      const created = await runCLI(['new', 'change', 'uses-default'], {
        cwd: tempDir,
      });
    ...
    it('should preserve an existing schema when forced init rejects an artifact', async () => {
      ...
      expect(JSON.parse(output as string)).toEqual({
        created: false,
        error: "Artefato desconhecido 'task'",
...
describe('schema command shell completion registry', () => {
  it('should have schema command in registry', async () => {
    ...
    expect(schemaCmd?.description).toBe('Gerencia esquemas de fluxo de trabalho [experimental]');
  });
