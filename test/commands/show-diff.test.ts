...
import { CHANGE_MESSAGES, SHOW_MESSAGES } from '../../src/messages/index.js';
...
describe('openspec show --diff', () => {
  ...
  it('text mode: shows proposal then MODIFIED diff and ADDED full text', () => {
    ...
    expect(output.indexOf('Improve auth.')).toBeLessThan(
      output.indexOf(CHANGE_MESSAGES.specDiffsHeading)
    );
  ...
  it('text mode: without --diff the output is the proposal, unchanged', async () => {
    ...
    expect(output).not.toContain(CHANGE_MESSAGES.specDiffsHeading);
  });
  ...
  it('text mode: shows warning when MODIFIED has no matching base', async () => {
    ...
    expect(output).toContain('Nenhum requisito correspondente encontrado');
  });
  ...
  it('text mode: reports when the change has no delta specs to diff', async () => {
    ...
    expect(output).toContain(CHANGE_MESSAGES.noDeltaSpecsToDiff('empty-change'));
    expect(output).not.toContain(CHANGE_MESSAGES.specDiffsHeading);
  });
  ...
  it('flags a MODIFIED header that differs from the main spec only in spacing', async () => {
    ...
    expect(output).toContain('apenas em caixa ou espaços');
    ...
    expect(modified.warning).toContain('apenas em caixa ou espaços');
  });
  ...
  it('propagates delta discovery failures instead of reporting no delta specs', async () => {
    ...
    expect(result.stdout).not.toContain('Nenhum spec de delta para comparar');
  });
  ...
  it('propagates main-spec read failures instead of reporting the spec missing', async () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).not.toBe('');
    expect(result.stdout).not.toContain('Não há spec principal em');
  });
  ...
  it('warns instead of inventing a diff when a MODIFIED spec has no main spec', async () => {
    ...
    expect(output).toContain('Não há spec principal em openspec/specs/billing/spec.md');
    ...
    expect(modified.warning).toContain('Não há spec principal em openspec/specs/billing/spec.md');
  });
  ...
  it('warns and ignores --diff when the item is a spec', async () => {
    ...
    expect(stderr).toContain(SHOW_MESSAGES.ignoringFlags('spec', 'diff'));
    expect(stderr).toContain('diff');
    expect(stdout).toContain('### Requirement: User login');
    expect(stdout).not.toContain(CHANGE_MESSAGES.specDiffsHeading);
  });
  ...
  it('text mode: distinguishes an empty diff from a missing base', async () => {
    ...
    expect(output).toContain(CHANGE_MESSAGES.noTextualChanges);
  ...
  it('RENAMED chain plus MODIFIED diffs against the original main name', async () => {
    ...
    expect(output).not.toContain('Nenhum requisito correspondente encontrado');
  });
});
