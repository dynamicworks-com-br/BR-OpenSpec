...
describe('non-English validation (#243)', () => {
  it('passes normally with guidance but still fails in strict mode', async () => {
    ...
    expect(normal.stdout).toContain('Especificação');
    expect(normal.stdout).toContain('é válida');
    ...
    expect(report.items[0].issues).toContainEqual(
      expect.objectContaining({
        level: 'WARNING',
        message: expect.stringContaining('deveria conter SHALL ou MUST'),
      })
    );
    ...
    expect(strictOutput).toContain('deveria conter SHALL ou MUST');
    expect(strictOutput).toContain('tem problemas');
  });
