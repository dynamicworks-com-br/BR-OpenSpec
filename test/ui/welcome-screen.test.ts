...
describe('welcome screen', () => {
  ...
  it('omits the quick start block when no onboarding workflow is installed', async () => {
    ...
    expect(output).toContain('Bem-vindo ao BR-OpenSpec');
  ...
  it.runIf(process.platform === 'darwin' || process.platform === 'linux')(
    ...
    async () => {
      ...
      const output = writtenOutput();
      expect(output).toContain('Bem-vindo ao BR-OpenSpec');
      // Static fallback: the frame is drawn exactly once, no repaint loop.
      expect(output).not.toMatch(/\x1b\[\d+A/);
    }
  );
  ...
  it('waits for Enter on the static path too (NO_COLOR)', async () => {
    ...
    expect(useKeypressMock).toHaveBeenCalledOnce();
    expect(writtenOutput()).toContain('Pressione Enter');
    // No cursor-up repaints: the frame is drawn exactly once.
    expect(writtenOutput()).not.toMatch(/\x1b\[\d+A/);
  });
  ...
  it('drops the Pressione Enter line when there is no TTY to wait on', async () => {
    ...
    expect(writtenOutput()).not.toContain('Pressione Enter');
  });
});
