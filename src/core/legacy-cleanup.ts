 ...
 *
 * Divergência do fork: além dos 12 nomes do upstream, o BR-OpenSpec gerava
 * `opsx-code-review.md` (o fork tem o workflow `code-review`). Sem essa entrada
 * o arquivo ficaria órfão para sempre no diretório global de prompts.
 */
const LEGACY_GLOBAL_CODEX_WORKFLOWS: Record<string, readonly WorkflowId[]> = {
  ...
  'opsx-code-review.md': ['code-review'],
...
export async function cleanupLegacyArtifacts(
  ...
  for (const filePath of detection.globalSlashCommandFiles) {
    if (!getManagedGlobalLegacyPromptMetadata(filePath)) {
      result.errors.push(LEGACY_CLEANUP_MESSAGES.skippedUnmanagedGlobalPrompt(filePath));
      continue;
    }
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const file of result.deletedFiles) {
      ...
      lines.push(
        replacementLabel
          ? LEGACY_CLEANUP_MESSAGES.removedFileReplacedBy(file, replacementLabel)
          : LEGACY_CLEANUP_MESSAGES.removedFile(file)
      );
    }
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const prompt of getLegacyGlobalPromptMatches(detection)) {
    const explanation = prompt.toolId
      ? LEGACY_CLEANUP_MESSAGES.explanationReplacedByToolSkills(`.${prompt.toolId}`)
      : LEGACY_CLEANUP_MESSAGES.explanationReplacedBySkills;
...
export function formatDeferredGlobalPromptSummary(detection: LegacyDetectionResult): string {
  ...
  const lines: string[] = [];
  lines.push(chalk.bold(LEGACY_CLEANUP_MESSAGES.deferredGlobalPromptsHeader));
  lines.push(chalk.dim(LEGACY_CLEANUP_MESSAGES.deferredGlobalPromptsSubheader));
  for (const prompt of deferredPrompts) {
    const toolLabel = prompt.toolId ? `${prompt.toolId}: ` : '';
    lines.push(LEGACY_CLEANUP_MESSAGES.deferredGlobalPromptItem(toolLabel, prompt.path));
  }
