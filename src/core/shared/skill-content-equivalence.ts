 ...
 *
 * Manter em sincronia com `ONBOARDING_MESSAGES.codexDualSkillReference`
 * (src/messages/index.ts), que é a frase gerada por
 * `transformToCodexCompatibleSkillReferences`.
 */
function toLegacyCodexReferences(content: string): string {
  return content.replace(
    /\$(openspec-[a-z0-9-]+) \(Codex\) ou \/\1 \(outros agentes\)/g,
