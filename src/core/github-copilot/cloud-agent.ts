/**
 * Suporte ao Copilot coding agent (nuvem) do GitHub
 *
 * Gera `copilot-setup-steps.yml` e `.github/agents/openspec.agent.md` quando a
 * ferramenta `github-copilot` é selecionada no init/update E o usuário optou
 * pelos arquivos de nuvem (opt-in). Esses arquivos permitem que o Copilot
 * coding agent hospedado no GitHub use a CLI do BR-OpenSpec no ambiente
 * efêmero dele.
 *
 * MANUTENÇÃO — reconhecimento de arquivo gerenciado: um arquivo só é
 * atualizado/removido quando seu conteúdo é EXATAMENTE o gerado (atual ou um
 * legado reconhecido, com CRLF normalizado). Ao alterar o texto PT-BR dos
 * templates em `COPILOT_CLOUD_AGENT_TEMPLATE_MESSAGES`, o corpo anterior
 * precisa entrar em `getLegacyCopilotCloudFileContents`; senão arquivos
 * gerados por versões anteriores do fork passam a ser tratados como
 * "customizados" e nunca mais são atualizados nem removidos.
 *
 * As funções `generateUpstream*` reproduzem VERBATIM (em inglês) os corpos
 * gerados pelo OpenSpec upstream. Elas existem só para reconhecimento: um
 * projeto inicializado com o upstream e migrado para o BR-OpenSpec tem seus
 * arquivos reconhecidos como gerenciados e atualizados para o conteúdo PT-BR.
 * NÃO traduzir essas funções: `replaceRequired` lança se a frase em inglês não
 * existir, e o mapa `COPILOT_CLOUD_FILE_CONTENTS` é avaliado no import.
 */
...
import {
  ...
  COPILOT_CLOUD_AGENT_TEMPLATE_MESSAGES,
} from '../../messages/index.js';

const COPILOT_TOOL_ID = 'github-copilot';
const OPENSPEC_MANAGED_MARKER = COPILOT_CLOUD_AGENT_TEMPLATE_MESSAGES.managedMarker;
/** Marcador usado pelo OpenSpec upstream — só para reconhecer arquivos legados. */
const UPSTREAM_MANAGED_MARKER =
 ...
 * This workflow pre-installs the BR-OpenSpec CLI in the Copilot coding agent's
...
function generateCopilotSetupStepsBody(): string {
  return COPILOT_CLOUD_AGENT_TEMPLATE_MESSAGES.setupStepsBody;
}

/**
 * Generate the .github/agents/openspec.agent.md custom agent file content.
 * This tells the GitHub Copilot coding agent how to use the BR-OpenSpec CLI.
 */
...
function generateCopilotAgentFileBody(includeManagedMarker = false): string {
  ...
  return COPILOT_CLOUD_AGENT_TEMPLATE_MESSAGES.agentFileBody(managedMarker);
}

// ─────────────────────────────────────────────────────────────────────────────
// Corpos do upstream (inglês, verbatim) — usados apenas para reconhecer
// arquivos gerados pelo OpenSpec upstream como gerenciados. Não traduzir.
// ─────────────────────────────────────────────────────────────────────────────

function generateUpstreamCopilotSetupStepsBody(): string {
...
export function generateUpstreamCopilotAgentFileBody(includeManagedMarker = false): string {
  const managedMarker = includeManagedMarker
    ? `<!-- ${UPSTREAM_MANAGED_MARKER} -->\n\n`
    : '';
...
export function generateUpstreamPreviousCopilotAgentFileBody(
  includeManagedMarker = false
): string {
  let content = generateUpstreamCopilotAgentFileBody();
...
function generateUpstreamLegacyCopilotAgentFileBody(): string {
  let content = generateUpstreamPreviousCopilotAgentFileBody();
  content = replaceRequired(
    content,
    `## Workflow
...
function replaceRequired(
  ...
  if (!content.includes(searchValue)) {
    throw new Error(COPILOT_CLOUD_AGENT_MESSAGES.cannotBuildContent(label));
  }
...
function getLegacyCopilotCloudFileContents(relPath: CopilotCloudFile): string[] {
  const upstreamSetupStepsBody = generateUpstreamCopilotSetupStepsBody();

  if (relPath === COPILOT_CLOUD_FILES.setupSteps) {
    return [
      // Variante PT-BR sem marcador (gerada por versões anteriores do fork).
      generateCopilotSetupStepsBody(),
      // Arquivos gerados pelo OpenSpec upstream (com e sem marcador).
      upstreamSetupStepsBody,
      `# ${UPSTREAM_MANAGED_MARKER}\n\n${upstreamSetupStepsBody}`,
    ];
  }
  ...
  return [
    ...
    // Arquivos gerados pelo OpenSpec upstream: atual, anterior e legado.
    generateUpstreamCopilotAgentFileBody(),
    generateUpstreamCopilotAgentFileBody(true),
    generateUpstreamPreviousCopilotAgentFileBody(),
    generateUpstreamPreviousCopilotAgentFileBody(true),
    generateUpstreamLegacyCopilotAgentFileBody(),
  ];
}
  ...
  while (true) {
    try {
      ...
      if (!stats.isDirectory()) {
        throw new Error(COPILOT_CLOUD_AGENT_MESSAGES.parentNotDirectory(candidate));
      }
      return;
    } catch (error) {
    ...
    if (parent === candidate) {
      throw new Error(COPILOT_CLOUD_AGENT_MESSAGES.cannotResolveAncestor(filePath));
    }
    candidate = parent;
  }
}
...
async function assertMissingOrRegularFile(filePath: string): Promise<void> {
  try {
    ...
    if (!stats.isFile()) {
      throw new Error(COPILOT_CLOUD_AGENT_MESSAGES.managedPathNotRegularFile(filePath));
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      throw error;
...
async function classifyCopilotAgentReconciliation(
  ...
  throw new Error(
    COPILOT_CLOUD_AGENT_MESSAGES.conflictingAgentProfiles(
...
// Opt-in
//
// Gerar um workflow do GitHub Actions dentro do `.github/` do usuário é
// invasivo e nos amarra ao formato de custom agent do Copilot (de propriedade
// do GitHub), então os arquivos de nuvem são opt-in em vez de um efeito
// colateral automático de selecionar a ferramenta Copilot. A decisão é
// persistida em openspec/config.yaml para que um `openspec update` não
// interativo (CI, agentes) a honre sem nunca perguntar.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * currently hold user-owned, non-managed content — i.e. files BR-OpenSpec will
...
/**
 * Return the managed cloud-file paths (relative to the project root) that
 * currently exist and hold BR-OpenSpec-generated content. Callers report this
