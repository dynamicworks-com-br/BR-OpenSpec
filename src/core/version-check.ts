...
import { VERSION_CHECK_MESSAGES } from '../messages/index.js';
...
export function buildCliUpdateLines(
  ...
  const lines = [VERSION_CHECK_MESSAGES.newerCliAvailable(OPENSPEC_VERSION, latestVersion)];
  ...
  if (installDir) {
    lines.push(VERSION_CHECK_MESSAGES.runningFrom(installDir));
  }
...
export function buildUpgradeCommandLines(
  ...
  if (isProjectLocalInstall(installDir, projectPath)) {
    ...
    lines.push(VERSION_CHECK_MESSAGES.updateProjectDependency(PACKAGE_NAME));
  } else {
    lines.push(`  ${GLOBAL_UPGRADE_COMMANDS[detectPackageManager(installDir)]}`);
  }

  lines.push(VERSION_CHECK_MESSAGES.rerunUpdateHint);
  return lines;
}
...
export async function offerCliUpgrade(latestVersion: string): Promise<UpgradeOutcome> {
  ...
  try {
    accepted = await confirm({
      message: VERSION_CHECK_MESSAGES.upgradePrompt(latestVersion),
      default: true,
    });
  } catch (error) {
  ...
  if (!installed) {
    console.log(chalk.yellow(VERSION_CHECK_MESSAGES.upgradeIncompleteLine1));
    console.log(chalk.yellow(VERSION_CHECK_MESSAGES.upgradeIncompleteLine2));
    return 'failed';
  }
  ...
  if (!version) {
    console.log(chalk.yellow(VERSION_CHECK_MESSAGES.upgradeUnconfirmed));
    return 'not-on-path';
  }
  if (compareVersions(version, OPENSPEC_VERSION) <= 0) {
    console.log(chalk.yellow(VERSION_CHECK_MESSAGES.upgradeStillReportsOld(version)));
    console.log(
      chalk.dim(
        binPath
          ? // We asked the installed copy directly, so PATH is not the story.
            VERSION_CHECK_MESSAGES.binUnchanged(binPath)
          : VERSION_CHECK_MESSAGES.stalePathInstallAnswering
      )
    );
    return 'not-on-path';
  }

  console.log(chalk.green(VERSION_CHECK_MESSAGES.upgradedTo(version)));
  return 'upgraded';
}
...
export async function rerunUpdateWithUpgradedCli(
  ...
  return new Promise((resolve) => {
    ...
    child.on('error', () => {
      ...
      console.log(chalk.yellow(VERSION_CHECK_MESSAGES.filesNotRegenerated));
      console.log(chalk.dim(VERSION_CHECK_MESSAGES.runUpdateToRegenerate));
      resolve(1);
    });
