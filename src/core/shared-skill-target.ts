...
/**
 * Returns the tool that currently owns this tool's physical skills root, or
 * `undefined` when the root is not shared (nobody else can lose work there).
 */
export function resolveSharedSkillTargetOwner(
  projectPath: string,
  toolId: string
): string | undefined {
  ...
  return reconcileSharedSkillTargets(projectPath, sharingRoot)[0]?.value;
}
...
export function isSharedSkillTargetActive(projectPath: string, toolId: string): boolean {
  ...
  const owner = resolveSharedSkillTargetOwner(projectPath, toolId);
  return owner === undefined || owner === toolId;
}
...
/**
 * Drops the ownership marker of a shared skills root, but only when `toolId`
 * still owns it: removing one tool must never hand the tree to a sibling that
 * never wrote it, nor leave a marker claiming a tool that has no skills left.
 *
 * @returns Whether a marker was actually removed
 */
export function clearSharedSkillTarget(projectPath: string, toolId: string): boolean {
  const tool = AI_TOOLS.find((candidate) => candidate.value === toolId);
  if (!tool?.skillsDir) return false;
  if (readSharedSkillTarget(projectPath, tool.skillsDir) !== toolId) return false;
