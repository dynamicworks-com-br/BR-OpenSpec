...
/**
 * Summarizes what a migration moved, e.g. "6 skills e 6 comandos".
 */
export function describeLegacyMigration(migration: LegacyToolMigration): string {
  const parts: string[] = [];
  if (migration.skillDirs > 0) {
    parts.push(MIGRATION_MESSAGES.skillCount(migration.skillDirs));
  }
  if (migration.commandFiles > 0) {
    parts.push(MIGRATION_MESSAGES.commandCount(migration.commandFiles));
  }
  return parts.join(' e ');
}
...
export function keptInPlaceNotice(migration: LegacyToolMigration): string | undefined {
  ...
  return MIGRATION_MESSAGES.keptInPlaceNotice(migration.keptInPlace, migration.from, migration.to);
}
...
export function legacyMigrationNotice(migration: LegacyToolMigration): string {
  if (migration.toolId === 'devin') {
    return MIGRATION_MESSAGES.legacyMigrationNoticeDevin(migration.from, migration.to);
  }
  return MIGRATION_MESSAGES.legacyMigrationNoticeGeneric(migration.from, migration.to);
}
