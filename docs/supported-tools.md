...
Codex is skills-only: BR-OpenSpec installs `.agents/skills/openspec-*/SKILL.md` for Codex even when delivery is set to `commands`, and it does not generate Codex custom prompt files. Existing BR-OpenSpec-managed skills under the legacy `.codex/skills` path are reconciled after their replacements are written; custom and divergent files are preserved.
...
| none — skills only | `/openspec-<skill>` | ForgeCode, MiniMax Code, Mistral Vibe, Trae, Zed Agent, shared `.agents` target |
...
MiniMax Code is a global skills-only integration. BR-OpenSpec writes only its
...
GitHub's [Copilot coding agent](https://docs.github.com/en/copilot/using-github-copilot/coding-agent) runs on GitHub in a GitHub Actions environment — separate from Copilot in your editor. BR-OpenSpec can set it up to use the `openspec` CLI by generating two files:

- `.github/workflows/copilot-setup-steps.yml` — installs `@dynamicworks/br-openspec` in the agent's environment
- `.github/agents/openspec.agent.md` — tells the agent how to drive BR-OpenSpec
...
| `openspec update` | Never prompts. Refreshes the files only if you opted in (or the project already has them). If you opted out, it removes BR-OpenSpec-managed cloud files. |

Your choice is saved in `openspec/config.yaml` as `githubCopilot.cloudAgent: true|false`, so non-interactive updates honor it. BR-OpenSpec only ever writes or removes files whose content it generated — if you customize `copilot-setup-steps.yml` or `openspec.agent.md`, or already have your own, it is left untouched (and `init`/`update` tell you so).
...
`.agents` root. If Codex is selected with Zed or `agents`, BR-OpenSpec keeps one
...
directory is Antigravity's former root, read only for migration.
...
Because `.agents/skills/` is shared by Antigravity, Codex, Zed Agent, and the
vendor-neutral target, it is worth knowing what BR-OpenSpec claims there:
...
directory is left alone. Treat the `openspec-*` names and marker as BR-OpenSpec's —
edits inside them are replaced on the next `openspec update`, the same as for
every other tool.

For pre-marker projects, BR-OpenSpec infers ownership from managed skill references:
