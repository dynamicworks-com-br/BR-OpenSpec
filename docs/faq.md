...
### What is BR-OpenSpec, in one sentence?
...
Because AI assistants are confident even when they're wrong. When the requirements live only in a chat thread, the AI fills gaps with guesses, and you find out after the code exists. BR-OpenSpec moves the agreement earlier, where mistakes are cheap to fix. See [Core Concepts at a Glance](overview.md) for the full case.
...
Existing codebases are the main event. BR-OpenSpec is brownfield-first: you do not document your whole app up front. You write specs only for what each change touches, and your specs fill in over time around the work you actually do. There's a dedicated guide: [Using BR-OpenSpec in an Existing Project](existing-projects.md).

### Is it tied to one AI tool?

No. BR-OpenSpec works with 30+ assistants, including Claude Code, Cursor, Devin Desktop, GitHub Copilot, Gemini CLI, Codex, and more. The full list and per-tool details are in [Supported Tools](supported-tools.md).
...
Each AI tool surfaces custom commands a little differently, and BR-OpenSpec spells them the way your tool loads the file it wrote. A command file named `opsx-propose.md` is typed `/opsx-propose`; one filed under `commands/opsx/` is typed `/opsx:propose`. Amazon Q's files are prompt-library entries, typed `@opsx-propose`. Tools that take skills instead of commands use the skill name — Codex skills are `$openspec-propose`, Kimi Code `/skill:openspec-propose`. The `openspec init` "Getting started" line already prints the right form for the tools you picked; the full table is in [How To Invoke](supported-tools.md#how-to-invoke).

### What's the difference between a skill and a command?

Both are files BR-OpenSpec writes so your assistant can run the workflow. Skills (`.../skills/openspec-*/SKILL.md`) are the newer cross-tool standard; commands (`.../commands/opsx-*`) are the older per-tool slash files. You don't need to pick. You just type the slash command, and BR-OpenSpec installs whichever your tool uses.
...
A spec that describes only what's changing, using `ADDED`, `MODIFIED`, and `REMOVED` sections, rather than restating the whole spec. It's how BR-OpenSpec handles edits to existing systems cleanly. See [Concepts](concepts.md#delta-specs).
...
BR-OpenSpec works best with high-reasoning models. The README recommends models like Codex 5.5 and Opus 4.8 for both planning and implementation. Also keep your context window clean: clear it before implementation for best results.

### Does BR-OpenSpec collect data?
...
Two steps. Upgrade the package (`npm install -g @dynamicworks/br-openspec@latest`), then run `openspec update` inside each project to refresh the generated skills and commands.

### How do I uninstall BR-OpenSpec?

There's no uninstall command, because it's just a global package plus files in your project. Remove the package (`npm uninstall -g @dynamicworks/br-openspec`), and optionally delete the `openspec/` directory and the generated tool files. Step-by-step, including what's safe to keep, is in [Installation: Uninstalling](installation.md#uninstalling).
