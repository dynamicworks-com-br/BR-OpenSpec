...
**The one thing to know: BR-OpenSpec has two kinds of commands, and they run in two different places.**
...
If you ever type `/opsx:propose` into your terminal and nothing happens, this page is why. You are talking to the wrong half of BR-OpenSpec. Slash commands are not terminal commands. They are instructions you give to your AI coding assistant, in the same chat box where you'd normally type "add a login form."

That single distinction is the most common stumbling block for new users, so let's make it crystal clear.

## The two halves

BR-OpenSpec is one project wearing two hats.
...
```bash
openspec init        # set up BR-OpenSpec in this project
...
You don't enter a special BR-OpenSpec mode. You just open your AI coding assistant like you always do, and type a slash command into the chat. The slash command *is* how you "enter" BR-OpenSpec. Your assistant recognizes it, loads the matching BR-OpenSpec skill, and starts following the workflow.
...
loads its files into a prompt library invoked with `@`, and the skill rows
...
When you run `openspec init` (or `openspec update`), BR-OpenSpec writes small files into your project so your AI tool can find the workflow. Depending on your tool and settings, these are **skills**, **commands**, or both.
...
- **Commands** live in places like `.cursor/commands/opsx-<id>.md` or `.claude/commands/opsx/<id>.md` — the layout is the tool's, and it decides how you type the command. They're the older per-tool slash command files. Codex's command files live in the global Codex home (`$CODEX_HOME/prompts/`), not your project.
...
By default, BR-OpenSpec installs the **core** set of slash commands:
...
There's also an **expanded** set for people who want finer control (`/opsx:new`, `/opsx:continue`, `/opsx:ff`, `/opsx:verify`, `/opsx:code-review`, `/opsx:bulk-archive`, `/opsx:onboard`). You turn it on with `openspec config profile`, then apply it with `openspec update`.
