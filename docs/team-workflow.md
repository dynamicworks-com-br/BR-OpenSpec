...
The short answer: a change is just files, and BR-OpenSpec never touches git. So it fits your existing workflow instead of replacing it. This page spells out the conventions that work well.

## One rule: BR-OpenSpec doesn't touch git

BR-OpenSpec reads and writes plain Markdown under `openspec/`. It never commits, branches, pushes, or pulls in your project. That means:
...
- **Everything below is convention, not enforcement.** BR-OpenSpec won't make you do it this way; it just fits cleanly.
...
Everything above assumes the plan lives in the code repo's own `openspec/` folder, which is the right default. When work genuinely spans several repos — one feature touching three services, or requirements one team owns and others consume — keep one `openspec/` per repo and coordinate by convention: matching change names across repos, links between the proposals, and each repo's specs describing its own side of the behavior.
