...
By default, BR-OpenSpec uses the `core` workflow profile (`propose`, `explore`, `apply`, `sync`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `code-review`, `bulk-archive`, `onboard`), configure them with `openspec config profile` and apply with `openspec update`.
