...
By default, BR-OpenSpec uses the `core` workflow profile (`propose`, `explore`, `apply`, `update`, `sync`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `code-review`, `bulk-archive`, `onboard`), configure them with `openspec config profile` and apply with `openspec update`.
...
| `/opsx:code-review` | Review diffs, branches, PRs, or files with project context (expanded workflow) |
